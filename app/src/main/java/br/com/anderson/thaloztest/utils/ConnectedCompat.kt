...
object ConnectedCompat {
    ...
    private val connectedCompatImpl: ConnectedCompatImpl

    init {
        connectedCompatImpl = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            MarshMallowImpl
        } else {
            BaseImpl
        }
    ...
    fun isConnected(connectivityManager: ConnectivityManager) =
        connectedCompatImpl.isConnected(connectivityManager)
